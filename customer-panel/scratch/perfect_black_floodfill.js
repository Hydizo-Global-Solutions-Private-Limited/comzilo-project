...
async function perfectBlackFloodFill() {
...
<body>
  ...
  <script>
    ...
    img.onload = () => {
      ...
      function isBlackBg(idx) {
        ...
        // Bottom under handle
        if (x < 300 && y > 665) return true;
        // Right outside cylinder
        if (x > 840) return true;
        // Base bottom curve
        if (y > 788) return true;
  ...
  if (canvasElement) {
    ...
    console.log('Flood fill complete:', outputPath);
  }

  await browser.close();
}

perfectBlackFloodFill().catch(err => {
  console.error(err);
  process.exit(1);
});
